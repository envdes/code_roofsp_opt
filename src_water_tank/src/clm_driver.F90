...
contains
  ...
  !-----------------------------------------------------------------------
  subroutine clm_drv(mon, tod, doalb, nextsw_cday, declinp1, declin, rstwr, nlend, rdate, rof_prognostic)
    !
    ! !DESCRIPTION:
    !
...
! --> J: add UrbSprinklingIrrigation
    use UrbSprinklingIrrigationMod, only : UrbSprinklingIrrigation
!JJ
! --> J: add q_table, q_state, q_action, q_reward_comp
     !use UrbQlearningMod, only : load_q_table, save_q_table, q_reward_nexstate, update_q_table
!JJ
    !
    ! !ARGUMENTS:
    implicit none
! --> J: add tod as input argument
    !real(r8),        intent(inout)   :: q_table(:,:) ! Q-table for Q-learning
...
! --> J: add q_table, q_state, q_action, q_reward_comp
     !real(r8), allocatable               :: q_table(:,:)
     integer                             :: q_state
     integer                             :: q_action
     real(r8)                            :: q_reward_comp
     real(r8)                            :: q_reward
     integer                             :: q_next_state
!JJ
...
! --> JJ: add UrbSprinklingIrrigation
       call t_startf('UrbSprinklingIrrigation')
       if (mon >= 6 .and. mon <= 9) then
          call UrbSprinklingIrrigation(  tod, &
             filter(nc)%num_nolakec, filter(nc)%nolakec, &
             temperature_inst, atm2lnd_inst, & 
             soilstate_inst, waterstate_inst, waterflux_inst, energyflux_inst)
       end if
       call t_stopf('UrbSprinklingIrrigation')
!JJ
...
! --> J: update qlearning
    ...
    !write(*,*) 'temperature_inst%t_grnd_col after:', temperature_inst%t_grnd_col
    !do nc = 1,nclumps
    !    ! ============================================================================
    !    ! Update Q-learning
    !    ! ============================================================================
    !    call t_startf('qlearning')
    !    !write(*,*) 'Q-learning'
    !    call q_reward_nexstate(tod, filter(nc)%num_nolakec, filter(nc)%nolakec, &
    !                        temperature_inst, atm2lnd_inst, & 
    !                        soilstate_inst, waterstate_inst, energyflux_inst, &
    !                        q_reward_comp,  q_reward, q_next_state)
    !    !write(*,*) 'Q-learning: reward computed', q_reward_comp
    !    !write(*,*) 'Q-learning: reward = ', q_reward
    !    call update_q_table(q_state, q_action, q_reward, q_next_state, q_table)
    !    !write(*,*) 'Q-learning: q_table updated'
    !    !call save_q_table(q_table)
    !    !write(*,*) 'Q-learning: q_table saved'
    !    !deallocate(q_table)
    !    !write(*,*) 'Q-learning: q_table deallocated'
    !    call t_stopf('qlearning')
    !end do
