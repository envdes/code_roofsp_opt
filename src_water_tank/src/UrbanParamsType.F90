...
!JJ
     real(r8), pointer :: lt_building_max  (:,:)
!JJ
...
!JJ
     real(r8), pointer     :: lt_building_max      (:)   ! lun maximum internal building air temperature (K)
!JJ
...
!JJ
    allocate(this%lt_building_max      (begl:endl))          ; this%lt_building_max      (:)   = nan
!JJ
...
!JJ
          this%lt_building_max(l) = urbinp%lt_building_max(g,dindx)
!JJ
...
!JJ
             this%lt_building_max(l) = 320.00_r8
!JJ
          else
...
!JJ
                urbinp%lt_building_max(begg:endg, numurbl), &
!JJ
                stat=ier)
       if (ier /= 0) then
...
!JJ
       call ncd_io(ncid=ncid, varname='LT_BUILDING_MAX', flag='read', data=urbinp%lt_building_max, &
            dim1name=grlnd, readvar=readvar)
       if (.not. readvar) then
          call endrun( msg=' ERROR: LT_BUILDING_MAX NOT on fsurdat file'//errmsg(sourcefile, __LINE__))
       end if
!JJ
...
!JJ
                  urbinp%lt_building_max(nl,n)        <= 0._r8 .or. &
!JJ
...
!JJ
       write(iulog,*)'lt_building_max:  ',urbinp%lt_building_max(nindx,dindx)
!JJ
