...
!JJ water tank
    real(r8) :: water_tank_size                            ! water tank size (mm)
    !-----------------------------------------------------------------------
...
!JJ water tank 
         ...
         qflx_water_tank_in  =>    waterflux_inst%qflx_water_tank_in_col  , & ! Output: [real(r8) (:)   ]  water tank flux (mm/s)            
!JJ
...
!JJ water tank
         OPEN(UNIT=30, FILE="/p/clmuapp/water_tank_size.txt", STATUS="OLD", ACTION="READWRITE")
         REWIND(30)
         read(30,*) water_tank_size
         close(30)
         if (col%itype(c) == icol_roof) then
            if (water_tank(c) + qflx_surf(c)*dtime > water_tank_size) then
               qflx_surf(c) = (water_tank(c) + qflx_surf(c)*dtime - water_tank_size)/dtime
               qflx_water_tank_in(c) = (water_tank_size - water_tank(c))/dtime
               water_tank(c) = water_tank_size
            else
               qflx_water_tank_in(c) = qflx_surf(c)
               qflx_surf(c) = 0.0_r8
               water_tank(c) = water_tank(c) + qflx_water_tank_in(c)*dtime
            endif
         else
            water_tank(c) = 0.0_r8
         endif
